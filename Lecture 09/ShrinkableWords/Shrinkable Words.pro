...
#
# @author Marty Stepp (+ Julie Zelenski, Keith Schwarz, Reid Watson and others)
# @version 2019/10/02
# - minor fixes (error handling on path chars, missing files), spurious xcopy errors
DEFINES += SPL_PROJECT_VERSION=20191002   # kludgy YYYYMMDD constant used by lib to know its version
...
win64 {
    !exists($$PWD/lib/addr2line64.exe) {
        ...
        message(*******************************************************************)
        warning(Stack traces unavailable. Project is missing addr2line64.exe)
    }
}
...
win32 {
    !exists($$PWD/lib/addr2line.exe) {
        ...
        message(*** Place that file into your lib/ folder and try again.)
        message(*******************************************************************)
        warning(Stack traces unavailable. Project is missing add2line.exe)
    }
}
...
win32 {
    ...
    # /c - continue on errors (spurious interference blocking build, to be investigated further JDZ)
    copy_resource_files.commands = xcopy /s /q /y /c ${QMAKE_FILE_IN}
} else {
...
# END OF FILE (this should be line #548; if not, your .pro has been changed!)
