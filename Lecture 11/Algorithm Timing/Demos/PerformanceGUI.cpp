...
namespace {
    ...
    const FontSize kFontSize(16);
    const Color kTitleColor = Color::fromHex(0x000080);

    /* Axis constants. */
    const Color kFontColor = Color::fromHex(0x555555); // Davy's gray
    const Color kAxisColor = Color::fromHex(0x555555);
    const Font kAxisFont(FontFamily::SANS_SERIF, FontStyle::NORMAL, 8, kFontColor);
    const size_t kMaxAxisLabels = 10;    // Max number of major tick marks on the X axis

    /* Content areas. */
    const double kHeaderHeight    = 50;
    const Font kHeaderFont(FontFamily::SERIF, FontStyle::BOLD_ITALIC, 24, kFontColor);
    ...
    const Font kLegendFont(FontFamily::SANS_SERIF, FontStyle::NORMAL, 12, kFontColor);
    ...
    void PerformanceGUI::stepTests() {
        ...
        mConsole->doWithStyle(kFontSize, [&] {
    ...
    void PerformanceGUI::startTests(const Timing::TimeTest& test) {
        ...
        mConsole->doWithStyle(kTitleColor, GColorConsole::BOLD_ITALIC, kFontSize, [&] {
...
GRAPHICS_HANDLER("Performance Visualizer", GWindow& window) {
    return make_shared<PerformanceGUI>(window);
}

CONSOLE_HANDLER("Performance Visualizer") {
    cout << "Confirm success." << endl;
}
#else
#include "GUI/MiniGUI.h"
#include "TimedFunctions.h"
